...
def analyze_gameday_pool_with_ids(ids=[], historical_id = 46, week='1.10.23', neuter=False, model='ensemble'):
    ...
    statspath = os.getcwd() + r"\fd_gpd\_historical\player_stats\by_week"
    stats = pd.read_csv(statspath + "\\" + '{0}.csv'.format(historical_id)) 
    stats.set_index('RylandID_master', inplace=True)

    df = picks.reset_index().set_index('name').join(stats[['proj_actpts']]).reset_index()
    ...
    top = df.loc[team_scores.index[-1]][['index','proj_actpts','proba_1', 'proba_%', 'proba_rank']] 
    corr = pd.concat([dflineup[['proj_actpts']].sum(), (dflineup[['proba_1']].first())], axis=1)
    corr['pct_proba'] = corr['proba_1'].rank(pct=True)
    top['pct_proba'] = corr.loc[top.index[0]].pct_proba
    corr['proj_actpts'].corr(corr['proba_1'])
